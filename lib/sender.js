...
const createSender = () => {
	...
	const handleReceiverMessage = (ackMsg, _, cb) => {
		...
		if (cbs.has(seq)) cbs.get(seq)()
		sender.emit('ack', seq)
		cb()
	}

	const send = (payload, cb = null) => {
		...
		queuedMsg = [seq, payload]

		if (Date.now() > tLastPayloadSent + rtt.get()) {
		...
		if (typeof cb === 'function') cbs.set(seq, cb)
		return seq
	}
